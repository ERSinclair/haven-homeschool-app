...
export default function EventsPage() {
  ...
  const [eventReminder, setEventReminder] = useState<ReminderConfig>({ offset: null, delivery: ['push', 'notification'] });
  const [showEventDatePicker, setShowEventDatePicker] = useState(false);
  const [eventDateMonth, setEventDateMonth] = useState(() => new Date().toISOString().slice(0, 7));
            ...
            <AppHeader
              ...
              right={
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => {
                      const url = `https://familyhaven.app/events?manage=${(selectedEvent as Event).id}`;
                      if (navigator.share) {
                        navigator.share({ title: (selectedEvent as Event).title, url }).catch(() => {});
                      } else {
                  ...
                  {(() => {
                    ...
                    if (ev.user_rsvp) return (
                      <div className="space-y-2">
                        ...
                        <button onClick={() => setShowReminderFor(showReminderFor === ev.id ? null : ev.id)} className="w-full py-1.5 text-xs text-emerald-600 font-medium hover:text-emerald-700">
                          {showReminderFor === ev.id ? 'Hide reminder' : '+ Set reminder'}
                        </button>
                        {showReminderFor === ev.id && (
                          <div className="bg-gray-50 rounded-xl p-3 space-y-3">
                            <ReminderPicker value={eventReminder} onChange={setEventReminder} />
                            {eventReminder.offset && (
                              <button onClick={async () => {
                                const session = getStoredSession();
                                if (!session) return;
                                const eventDate = new Date(ev.event_date + 'T' + (ev.event_time || '09:00') + ':00');
                                const remindAt = new Date(eventDate.getTime() - offsetToMs(eventReminder.offset!));
                                if (remindAt > new Date()) {
                                  await fetch(`${supabaseUrl}/rest/v1/reminders`, {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json', 'apikey': supabaseKey!, 'Authorization': `Bearer ${session.access_token}`, 'Prefer': 'return=minimal' },
                                    body: JSON.stringify({ user_id: session.user.id, type: 'event', target_id: ev.id, target_title: ev.title, remind_at: remindAt.toISOString(), delivery: eventReminder.delivery }),
                                  }).catch(() => {});
                                  setReminderSaved(true);
                                  setShowReminderFor(null);
                                  setTimeout(() => setReminderSaved(false), 3000);
                                }
                              }} className="w-full py-2 bg-emerald-600 text-white text-xs font-semibold rounded-xl">
  ...
  return (
    ...
    <div className="min-h-screen bg-transparent">
      <div className="max-w-md mx-auto px-4 pb-8 pt-2">
        ...
        {/* Radius nudge — show at bottom of discover list */}
        {mainTab === 'discover' && eventsViewMode !== 'calendar' && (() => {
          ...
          return (
            <div className="text-center py-6 text-sm text-gray-400">
              ...
              <p className="mt-1">Try increasing your search radius.</p>
              <a href="/settings" className="mt-3 inline-flex items-center justify-center group">
                <svg className="w-6 h-6 text-gray-400 group-hover:text-emerald-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 010 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 010-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0Z" />
                </svg>
                    ...
                    {editingEventDate ? (
                      <div className="space-y-2">
                        <div className="relative">
                          <button
                            type="button"
                            onClick={() => setShowEventDatePicker(v => !v)}
                            className="w-full px-3 py-2 border border-gray-200 rounded-lg bg-white text-gray-900 text-sm text-left hover:border-emerald-400 transition-colors"
                          >
                            {tempEventDate ? new Date(tempEventDate + 'T12:00:00').toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Select date'}
                          </button>
                          {showEventDatePicker && (
                            <DatePickerModal
                              value={tempEventDate}
                              onChange={v => { setTempEventDate(v); setShowEventDatePicker(false); }}
                              onClose={() => setShowEventDatePicker(false)}
                              minDate={new Date().toISOString().slice(0, 10)}
                              month={eventDateMonth}
                              onMonthChange={setEventDateMonth}
                            />
                          )}
                        </div>
