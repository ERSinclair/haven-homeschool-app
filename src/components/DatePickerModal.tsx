...
export default function DatePickerDropdown({ value, onChange, onClose, minDate, maxDate, month, onMonthChange }: Props) {
  ...
  return (
    <div
      ref={ref}
      className="absolute left-0 mt-1 z-50 bg-white/60 backdrop-blur-md border border-white/40 rounded-2xl shadow-xl p-3 w-64"
    >
      {/* Month nav */}
      ...
      {value && (
        <button type="button" onClick={() => { onChange(''); onClose(); }} className="mt-2 w-full text-[11px] text-gray-400 hover:text-red-400 text-center">
          Clear
        </button>
      )}
    </div>
